...
// Version con fork
// se compila con: gcc descompresorFork.c -o descompresorFork
// se corre con: ./descompresorFork salida.huff descomprimidosFork/
...
int main(int argc, char* argv[]) {
    ...
    // Inicia los procesos para descomprimir los archivos
    struct timeval start, end;
    gettimeofday(&start, NULL);
    // crea un proceso por archivo a descomprimir
    pid_t* pids = (pid_t*)malloc(totalArchivos * sizeof(pid_t));
    for (int f = 0; f < totalArchivos; f++) {
        pid_t pid = fork();
        if (pid == 0) {
            // proceso hijo
            FILE* archivo = fopen(archivoLeido, "rb");
            if (!archivo) exit(2);
            fseek(archivo, offsets[f], SEEK_SET);
            char ruta[1024];
            snprintf(ruta, sizeof(ruta), "%s/%s", directorio, nombres[f]);
            FILE* archivoSalida = fopen(ruta, "wb");
            if (!archivoSalida) { fclose(archivo); exit(3); }
            descomprimir_archivo(archivo, archivoSalida, raiz, tamanos[f]);
            fclose(archivo);
    ...
    for (int f = 0; f < totalArchivos; f++) free(nombres[f]);
    free(nombres);
    free(tamanos);
    free(offsets);
    free(pids);
